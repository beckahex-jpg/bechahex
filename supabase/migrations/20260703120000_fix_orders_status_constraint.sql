/*
  The live orders_status_check only allowed pending/processing/completed/
  cancelled, but the app writes 'confirmed' (checkout after payment),
  'shipped' (seller shipping), 'delivered' (buyer confirmation) and 'paid'
  (admin payment management) — all of those UPDATEs were silently failing.
  Widen the constraint to the statuses the code actually uses, and require
  payment before mark_order_shipped will flip an order to shipped.
*/
...
BEGIN
  ...
  -- Checkout creates the order before payment completes; never let an
  -- unpaid order be shipped.
  IF v_order.payment_status <> 'paid' THEN
    RAISE EXCEPTION 'This order has not been paid yet' USING ERRCODE = '55000';
  END IF;
  ...
  VALUES (
    ...
    'order_shipped',
    'Order Shipped!',
    -- the live orders table has no order_number column; use the short id
    'Your order #' || substr(v_order.id::text, 1, 8) || ' has been shipped via ' || v_company || '. Tracking number: ' || v_tracking,
    jsonb_build_object(
      'order_id', v_order.id,
      'tracking_number', v_tracking,
      'shipping_company', v_company
