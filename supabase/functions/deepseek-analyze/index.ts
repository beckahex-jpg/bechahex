...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!DEEPSEEK_API_KEY) {
      console.error("DEEPSEEK_API_KEY is not configured in Supabase secrets");
      return new Response(
        JSON.stringify({
          error: "DEEPSEEK_API_KEY is not configured. Please add it to Supabase Dashboard > Project Settings > Edge Functions > Secrets"
        }),
        {
          status: 500,
...
Provide complete product details in JSON format:
{
  ...
  "suggestions": ["improvement suggestions"]
      ...
      case "analyze_from_text":
        systemPrompt = "You are a product analysis expert. Analyze the product description and generate complete product information including pricing.";
        userPrompt = `Analyze this product description and generate complete product information:
...
Respond with ONLY valid JSON in this exact format:
{
  ...
  "colors": ["visible or mentioned colors in the product"],
  "brandInfo": "brand name if mentioned or recognizable",
  "tags": ["5-7 relevant search keywords"],
  "estimatedPrice": number,
  "priceReasoning": "1-2 sentences explaining the price estimate"
}`;
        break;
    ...
    const maxTokens = action === "analyze_from_text" ? 800 : 300;
