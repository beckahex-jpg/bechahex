...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.image) {
      throw new Error('يجب تقديم صورة للتحليل');
    }
    ...
    const prompt = `أنت خبير في تحليل المنتجات من الصور لمنصة خيرية. حلل هذا المنتج من الصورة:

**مهمتك:**
1. تحليل المنتج من الصورة
2. اقتراح اسم جذاب ومحترف (بالعربية)
3. كتابة وصف تسويقي منسق (بالعربية، 2-3 جمل)
4. تحديد الفئة المناسبة
5. تقييم حالة المنتج من الصورة
6. اقتراح 3 أسعار بالدولار:
   - رمزي ($1-$5): للتبرع للمنصة
   - عادل (40-60% من سعر السوق): سعر منصف
   - سوق (70-90% من سعر السوق): سعر عالي
7. رسالة تشجيعية لطيفة للتبرع (جملة واحدة)
