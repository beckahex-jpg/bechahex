...
export default function MyProductsPage() {
  ...
  const loadProducts = async () => {
    try {
      ...
      const publishedProducts = enrichedData.filter(p => p.status === 'approved');
      for (const product of publishedProducts) {
        ...
        if (ordersData) {
          statsData.totalSales += ordersData.reduce((sum, item) =>
  ...
  const handleDelete = async (productId: string) => {
    ...
    try {
      ...
      if (product?.status === 'approved' && product.product_id) {
        ...
        if (productError) console.error('Error deleting product:', productError);
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4">
      ...
      {showViewModal && viewingProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-6xl w-full my-8 flex flex-col shadow-2xl" style={{maxHeight: 'calc(100vh - 4rem)'}}>
            ...
            <div className="px-6 py-6 space-y-4 overflow-y-auto flex-1" style={{maxHeight: 'calc(100vh - 18rem)'}}>
            ...
            <div className="px-6 py-5 bg-gray-50 border-t border-gray-200 rounded-b-2xl flex gap-4 justify-end shadow-inner flex-shrink-0">
              <button
                onClick={() => setShowViewModal(false)}
                className="px-8 py-3 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition font-semibold text-base flex items-center gap-2"
                disabled={saving}
              >
                <X className="w-5 h-5" />
                Close
              ...
              <button
                ...
                disabled={saving}
                className="px-8 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-lg hover:from-emerald-700 hover:to-emerald-800 transition disabled:opacity-50 flex items-center gap-2 font-semibold text-base shadow-lg"
              >
