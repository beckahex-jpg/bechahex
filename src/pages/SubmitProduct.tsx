          ...
          <div className="space-y-6">
            ...
            {formData.submission_type === 'donation' && (
              <div className="bg-green-50 border-2 border-green-300 rounded-xl p-4">
                <label className="block text-sm font-semibold text-gray-900 mb-2">
                  Product Sale Price (USD) <span className="text-red-600">*</span>
                </label>
                <input
                  type="number"
                  step="0.01"
                ...
                <div className="mt-3 bg-white border border-green-300 rounded-lg p-3">
                  ...
                  <ul className="text-sm text-green-700 space-y-1 mr-6">
                    ...
                    <li>• 100% goes to charity ❤️</li>
                  </ul>
                </div>
                <p className="text-sm text-green-700 mt-2 flex items-start gap-2">
                  <span>💡</span>
                  <span>Enter the actual market value of your product. Buyers will pay this full price to support charity.</span>
                </p>
              </div>
            )}
            ...
            {formData.submission_type === 'symbolic_sale' && (
              <>
                <div className="bg-blue-50 border-2 border-blue-300 rounded-xl p-4">
                  <label className="block text-sm font-semibold text-gray-900 mb-2">
                    Original Product Price (USD) <span className="text-red-600">*</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
                  ...
                  <p className="text-sm text-blue-700 mt-2 flex items-start gap-2">
                    <span>💡</span>
                    <span>This is the actual market value that will be shown to buyers</span>
                  </p>
                </div>

                <div className="bg-blue-50 border-2 border-blue-300 rounded-xl p-4">
                  <label className="block text-sm font-semibold text-gray-900 mb-2">
                    Your Symbolic Price (USD) <span className="text-red-600">*</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
                  ...
                  <p className="text-sm text-blue-700 mt-2 flex items-start gap-2">
                    <span>🔒</span>
                    <span>This is the symbolic amount you want to receive. This amount is private and only visible to admins. The rest goes to charity.</span>
                  </p>
                </div>

                <div>
                  <label className="block text-sm font-semibold text-gray-900 mb-2">
                    Public Sale Price (USD) <span className="text-red-600">*</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
                  ...
                  <p className="text-sm text-blue-600 mt-2">
                    💡 This is the price buyers will pay. Suggested: between your symbolic price and original price
                  </p>
                </div>
              </>
            )}
            ...
            {formData.submission_type === 'public_sale' && (
              <>
                <div>
                  <label className="block text-sm font-semibold text-gray-900 mb-2">
                    Sale Price (USD) <span className="text-red-600">*</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-4 sm:py-6 lg:py-8 pb-24 lg:pb-8">
      <div className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8">
        ...
        <div className="mb-4 sm:mb-6 bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl p-4 sm:p-6 text-white shadow-lg">
          <div className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-3 sm:gap-0">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-purple-100 truncate">Automatic analysis using Gemini & DeepSeek</p>
              </div>
            </div>
            <button
              onClick={() => setShowAIUpload(true)}
              className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-white text-purple-600 rounded-lg sm:rounded-xl font-bold text-sm sm:text-base hover:bg-purple-50 transition shadow-lg active:scale-95"
            >
              Try Now
            </button>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-4 sm:px-6 py-6 sm:py-8 text-white">
            <button
            ...
            <p className="text-blue-100 mt-1 text-xs sm:text-sm">Share your products and make a difference</p>
          </div>
                ...
                return (
                  <div key={step.id} className="flex-1 relative">
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center mb-1 sm:mb-2 transition ${
                          isCompleted
                    ...
                    {index < STEPS.length - 1 && (
                      <div
                        className={`absolute top-4 sm:top-5 lg:top-6 left-1/2 w-full h-0.5 -z-10 transition ${
          ...
          <div className="px-4 sm:px-6 py-6 sm:py-8">{renderStep()}</div>

          <div className="px-4 sm:px-6 py-4 sm:py-6 bg-gray-50 border-t border-gray-200">
            <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-4">
              <button
                onClick={() => navigate('/dashboard')}
                className="px-4 sm:px-6 py-2.5 sm:py-3 border-2 border-gray-300 text-gray-700 rounded-lg font-semibold text-sm sm:text-base hover:bg-gray-100 transition order-2 sm:order-1 active:scale-95"
              >
                Cancel
              </button>
              <div className="flex gap-2 sm:gap-3 order-1 sm:order-2">
                {currentStep > 1 && (
                  <button
                    ...
                    className="flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 bg-white border-2 border-blue-600 text-blue-600 rounded-lg font-semibold text-sm sm:text-base hover:bg-blue-50 transition active:scale-95"
                  >
                    Previous
                  </button>
                )}
                {currentStep < STEPS.length ? (
                  <button
                    onClick={() => setCurrentStep(currentStep + 1)}
                    disabled={!canProceed()}
                    className={`flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg font-semibold text-sm sm:text-base transition active:scale-95 ${
                      canProceed()
                  ...
                  <button
                    ...
                    className={`flex-1 sm:flex-none px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg font-semibold text-sm sm:text-base transition flex items-center justify-center gap-2 active:scale-95 ${
                      loading || !canProceed()
                        ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        <span className="text-sm sm:text-base">Submit Product</span>
                      </>
                    )}
                  </button>
                )}
