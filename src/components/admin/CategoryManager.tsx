...
export default function CategoryManager({ searchQuery = '' }: CategoryManagerProps) {
  ...
  const iconOptions = [
    ...
    'Dumbbell', 'Wrench', 'Hammer', 'Paintbrush', 'Lightbulb', 'Zap',
    'Flower2', 'Trees', 'Sofa', 'Armchair', 'Gem', 'Crown'
  ];
  ...
  const getIcon = (iconName: string) => {
    if (!iconName) return <Icons.Package className="w-8 h-8" />;

    const capitalizedIconName = iconName.charAt(0).toUpperCase() + iconName.slice(1);
    const IconComponent = (Icons as any)[iconName] || (Icons as any)[capitalizedIconName] || Icons.Package;
              ...
              <div>
                    ...
                    {iconOptions.map((iconName) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-4 border-2 rounded-lg hover:border-emerald-500 transition flex flex-col items-center gap-1 ${
                ...
                {formData.icon && (
                  <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center gap-3">
                    {(() => {
                      const IconComponent = (Icons as any)[formData.icon] || Icons.Package;
                      return <IconComponent className="w-8 h-8 text-blue-600" />;
                    })()}
                    <div>
                      <p className="text-sm font-medium text-blue-900">Selected Icon:</p>
                      <p className="text-sm text-blue-700">{formData.icon}</p>
                    </div>
                  </div>
                )}
              </div>
