...
import { Settings, Bell, Shield, User, Save, Mail, MessageSquare, Package, Cookie, Trash2, Loader2, ShoppingBag, Truck, Star, ShoppingCart, Landmark, Wallet } from 'lucide-react';
...
export default function SettingsPanel() {
  ...
  const loadSettings = async () => {
    try {
      ...
      const { data: payout } = await supabase
        ...
        .select('method, bank_name, account_holder_name, iban, paypal_email')
        .eq('user_id', user?.id)
        .maybeSingle();
  ...
  const handleSavePayoutDetails = async () => {
    if (payoutDetails.method === 'bank_transfer' && (!payoutDetails.bank_name.trim() || !payoutDetails.account_holder_name.trim() || !payoutDetails.iban.trim())) {
      showError('Please fill in the bank name, account holder name and IBAN / account number.');
      return;
    }
    if (payoutDetails.method === 'paypal' && !payoutDetails.paypal_email.trim()) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          bank_name: payoutDetails.bank_name.trim() || null,
          account_holder_name: payoutDetails.account_holder_name.trim() || null,
          iban: payoutDetails.iban.trim() || null,
          paypal_email: payoutDetails.paypal_email.trim() || null,
        }, { onConflict: 'user_id' });
          ...
          <div className="p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Where we send your earnings when you sell on Beckah</p>
              </div>
            </div>
            <p className="text-xs text-gray-500 mb-6">
              These details are only visible to you and the Beckah admin team, and are used to transfer your seller earnings.
            </p>

            <div className="grid grid-cols-2 gap-3 mb-5">
              <button
                type="button"
                onClick={() => setPayoutDetails({ ...payoutDetails, method: 'bank_transfer' })}
              ...
              <button
                type="button"
                onClick={() => setPayoutDetails({ ...payoutDetails, method: 'paypal' })}
                className={`flex items-center justify-center gap-2 rounded-xl border-2 px-4 py-3 text-sm font-semibold transition ${
            ...
            {payoutDetails.method === 'bank_transfer' ? (
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Bank Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Account Holder Name</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">IBAN / Account Number</label>
                  <input
                    type="text"
                    value={payoutDetails.iban}
                    maxLength={64}
                    onChange={(e) => setPayoutDetails({ ...payoutDetails, iban: e.target.value })}
                    placeholder="e.g. LB62 0999 0000 0001 0019 0122 9114"
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-mono"
                  />
                </div>
              </div>
            ) : (
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">PayPal Email</label>
                <input
                  type="email"
            ...
            <div className="flex justify-end pt-6 border-t border-gray-200 mt-6">
              <button
                ...
                className="flex items-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-8 py-3 rounded-xl font-semibold transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
              >
                {savingPayout ? (
                  <>
                    <Loader2 className="w-5 h-5 animate-spin" />
